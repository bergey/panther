...
-- |
-- Module      :  Solve
-- Copyright   :  (c) 2011 diagrams-lib team (see https://github.com/diagrams/diagrams-lib/blob/master/LICENSE)
-- License     :  BSD-style (see LICENSE)
--
-- Exact solving of low-degree (n <= 4) polynomials.
-- Taken from diagrams-lib package, per copyright above.
--
-----------------------------------------------------------------------------
module Solve
       ( quadForm
...
cubForm' toler a b c d
 ...
 where delta  = 18*a*b*c*d - 4*b^3*d + b^2*c^2 - 4*a*c^3 - 27*a^2*d^2
       ...
       trig k = 2 * sqrt(-p/3) * cos(phi - k*2*pi/3) - b/(3*a)
       cubert x | x < 0     = -((-x)**(1/3))
                | otherwise = x**(1/3)
